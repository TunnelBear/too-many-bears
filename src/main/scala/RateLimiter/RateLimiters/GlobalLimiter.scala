...
import RateLimiter.Strategies.{BaseIPStrategy, GlobalStrategy, IPStrategy}
...
case class GlobalLimiter(limit: Long, expiry: Long)(implicit rateLimiterStorage: RateLimiterStorage, executionContext: ExecutionContext) extends BaseRateLimiter {
  ...
  override def allow: Future[Boolean] = {
    GlobalStrategy(Identifier, limit, expiry).allow
  }

  override def increment: Future[Unit] = {
    GlobalStrategy(Identifier, limit, expiry).increment()
  }
